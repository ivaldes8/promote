...
const updateSkill = asyncHandler(async (req, res) => {
  ...
  if (skill.user.toString() !== req.user.id && req.user.role !== "Admin") {
    res.status(400);
    throw new Error(
      "Skills can only be modified by the user who create them and for the administrator"
    );
  }
...
const deleteSkill = asyncHandler(async (req, res) => {
  ...
  if (skill.user.toString() !== req.user.id && req.user.role !== "Admin") {
    res.status(400);
    throw new Error(
      "Skills can only be deleted by the user who create them and for the administrator"
    );
  }
